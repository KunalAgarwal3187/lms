...
//   console.log(course);
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const { data } = await axios.put(backendUrl+`/api/educator/update-courses/${courseId}`,formData,{ headers: { Authorization: `Bearer ${token}` } })

      if(data.success){
        toast.success(data.message)
        navigate("/educator/update-courses")
      }else{
        toast.error(data.message)
      }
    } catch (error) {
    ...
    <div className='h-screen overflow-scroll flex flex-col  items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        <div className='flex flex-col gap-1'>
          <p>course Title</p>
          <input type="text" required onChange={e => setCourseTitle(e.target.value)} value={courseTitle} placeholder='Type here' className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500' />
        </div>
        <div className='flex flex-col gap-1'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1'>
            <p>Course Price</p>
            <input type="number" required onChange={e => setCoursePrice(e.target.value)} value={coursePrice} placeholder='0' className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500 w-28' />
          </div>
          <div className='flex md:flex-row flex-col items-center gap-3'>
            <p>Course Thumbnail</p>
            <label htmlFor="thumbnailImage" className='flex items-center gap-3'>
              <img src={assets.file_upload_icon} alt="" className='p-3 bg-blue-500 rounded' />
              <input name='image' type="file" id='thumbnailImage' onChange={e => setImage(e.target.files[0])} accept='image/*' hidden />
        ...
        <div className='flex flex-col gap-1'>
          <p>Discount %</p>
          <input type="number" placeholder='0' required onChange={e => setDiscount(e.target.value)} value={discount} min={0} max={100} className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500 w-28' />
        </div>
